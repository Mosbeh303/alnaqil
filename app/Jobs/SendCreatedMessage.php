...
class SendCreatedMessage implements ShouldQueue
{
   ...
   public function handle()
    {
        ...
        try {
            $message = getSettingValue('creating_shipment_message');
            ...
            if ($message) {
                ...
                $receiver = Receiver::where('phone', $this->shipment->receiver_phone)->firstOrFail();

                if ($receiver && $receiver->location_adress==null || $receiver->location_adress=='') {
                    $url = url('receivers/' . base64_encode($receiver->phone) . '/create-additional-data');
                    $message .= ', الرجاء إكمال بيانات عنوانك عبر الرابط التالي:  ' . $url;
                }
