<template>
  <div class="flash fixed top-4 left-4 w-72 transition ease-in-out delay-150 z-50">
    <div
      v-if="$page.props.flash.success && show"
      class="transition-all ease-in-out delay-1000 shadow-lg flex items-center justify-between mb-8 gap-6 bg-green-500 rounded"
    >
      <div class="flex items-center">
        <svg
          class="flex-shrink-0 mr-4 ml-2 w-4 h-4 fill-white"
      ...
      <button type="button" class="group ml-2 p-2" @click="show = false">
        <svg
      ...
      <div class="flex items-center">
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.props.errors).length === 1">{{
            $page.props.language.flash_error
          }}</span>
          <span v-else
            >{{$page.props.language.flashErrorAfterCountingErrors }}
            {{ Object.keys($page.props.errors).length }}
            {{$page.props.language.flashErrorBeforeCountingErrors }}</span
          >
        </div>
      </div>
      <button type="button" class="group ml-2 p-2" @click="show = false">
